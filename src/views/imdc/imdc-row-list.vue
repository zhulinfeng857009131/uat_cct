    ...
    <el-container class="cont-conar cont-addjjfa" style="padding: 0 10px;min-height: 500px;">
      <el-container>
        ...
        <el-row class="cct-col-divl" :gutter="20" style="padding: 10px 0;background-color: #a5a9ac;color:#fff;margin :0 1px 20px;border: 1px solid #cfcfcf;">
          <el-col :span="8">
            <div>项目名称：<span> {{titleData.projectName }}</span></div>
        ...
        <div style="background-color: #5f686d;padding: 8px 20px;">
          ...
          <el-select v-model="Mini" placeholder="请选择IMDC Mini典型方案" @change="chooseQskuType('Mini',$event)" style="margin-right: 15px;width: 300px;">
            <el-option v-for="item_mini in qskuType.Mini" :value="item_mini.paramters" :label="item_mini.typeNum">
            </el-option>
          </el-select>
          <el-select v-model="One" placeholder="请选择IMDC One典型方案" @change="chooseQskuType('One',$event)" style="margin-right: 15px;width: 300px;">
              ...
              <el-col :span="8" v-for="item in navItem" style="margin: 0 0 15px;">
                <div style="padding: 10px 15px;border: 1px solid #cdcdcd;height: 130px;">
                  ...
                  <div v-for="data in item.sndImdccodeList" :class="'aaa'+ item.sndImdccodeList.length" style="display: table;margin: 8px 0;width: 100%;">
                    ...
                    <el-input style="width: 100%;" v-if="data.type =='0'" v-model="formdata[data.code]" placeholder="0"
                      :disabled="data.fill" onkeyup="value=value.replace(/[^\d]/g,'')" @blur="dataNavItem(data.code,$event)" />
                  </div>
                </div>
              </el-col>
            </el-row>
            ...
            <el-row :gutter="20">
              ...
              <el-col :span="6" style="margin: 0 0 15px;">
                <div style="padding: 10px 15px;border: 1px solid #cdcdcd;height: 130px;">
                  ...
                  <p style="font-size: 12px;color: #B4BCCC;">说明：
                    IMDC产品自带保修1年，如需要延长保修时请随产品单独选购。如一台IMDC需要多年延保，需要几年延保则购买几次即可。</p>
                </div>
              </el-col>
              <el-col :span="6" style="margin: 0 0 15px;">
                <div style="padding: 10px 15px;border: 1px solid #cdcdcd;height: 130px;">
        ...
        <div id="showIMDC" :class="showIMDCClass=='true'?'showIMDCshow':'showIMDChide'">
          ...
          <div style="height: 650px;width:80%;background-color: #f4f4f4;margin: 0 auto;text-align: center; vertical-align: middle; position: relative;top: 50%;margin-top: -350px;">
            <img src="@/assets/img/imdcClose.png" style="border-radius: 50%;position: absolute;top: 10px;right: 10px;cursor: pointer;"
              @click="showIMDCc">
            <div style="height: 650px;">

              <el-main style="padding: 0;height: auto; padding: 15px 0 15px 15px; border: 1px solid rgb(205, 205, 205); background-color: rgb(255, 255, 255); margin-bottom: 20px;">
                <div style="font-weight: bold;height: 50px;line-height: 50px;margin-bottom: 20px;padding-left:20px;background-color:#f4f4f4;">
					    操作提示
					    监控模块<input v-model="table.color1len">
					    配电箱<input v-model="table.color2len">
					    UPS<input v-model="table.color3len">
					    电池<input v-model="table.color4len">
					    IT柜数量:

              <el-select v-model="table.rdpNum" placeholder="请选择" @change="chooseQskuType('Mini',$event)" style="margin-right: 15px;width: 300px;">
                <el-option :value="3" label="1托3"></el-option>
                <el-option :value="4" label="1托4"></el-option>
                <el-option :value="5" label="2托5"></el-option>
                <el-option :value="6" label="2托6"></el-option>
                <el-option :value="7" label="2托7"></el-option>
                <el-option :value="8" label="2托8"></el-option>
              </el-select>
					    <input type="button" @click="tablesub()" value="查询">
					  </div>
                <el-main style="height: auto;background-color: #f4f4f4;margin-top: 0;text-align: center; vertical-align: middle;">
                  <div class="table-title1 ul-list ul-list-small">
                    <ul>
                      <li><span style="background-color: #000000;"></span>柜体</li>
                      <li><span style="background-color: #894916;"></span>监控模块，2U</li>
                      <li><span style="background-color: #d156b3;"></span>20K 1+1配电箱，12U</li>
                      <li><span style="background-color: #8c60b9;"></span>20K UPS，6U</li>
                      <li><span style="background-color: #266ecf;"></span>rPDU * 6，0U</li>
                      <li><span style="background-color: #26cf91;"></span>20K 电池，2U</li>
                    </ul>
                  </div>
                  <div class="table-title1 imdcrow-char" style="width: 80%;position: relative;">
                    <div style="overflow-x: scroll;width: 100%; display: flex;">
                    <div class="imdcrow-char-left">
                      <img src="@/assets/img/incdrow-z.png" class="rowimg" alt="">
                      <!-- <img src="@/assets/img/imdcrow-IT.png" class="rowimg" alt=""> -->
                      <h3>主控柜</h3>
                      <div style="position: absolute;top: 57px;left: 13px;">
                        <template v-if="table.color1len > 0" v-for="i in Number(table.color1len)">
                        <img src="@/assets/img/imdcrow-jk.png" alt="">
                        </template>
                        <template v-if="table.color2len > 0" v-for="i in Number(table.color2len)">
                        <img src="@/assets/img/imdcrow-pdx.png" alt="">
                        </template>
                        <template v-if="table.color3len > 0" v-for="i in Number(table.color3len)">
                        <img src="@/assets/img/imdcrow-ups.png" alt="">
                        </template>
                      </div>
                      <div style="position: absolute;bottom: 52px;left: 13px;">
                        <template v-if="table.color4len > 0" v-for="i in Number(table.color4len)">
                        <img src="@/assets/img/imdcrow-dc.png" alt="">
                        </template>
                      </div>
                    </div>
                    <template  v-if="table.rdpNum > 0 && i < table.rdpNum" v-for="i in Number(table.rdpNum)">
                      <div class="imdcrow-char-right" v-if="i == 1 && (table.rdpNum == 5 || table.rdpNum == 6)">
                        <img src="@/assets/img/imdcrow-kt.png" class="rowimg" alt="">
                        <h3>列间空调</h3>
                      </div>
                      <div class="imdcrow-char-right">
                        <img src="@/assets/img/imdcrow-IT.png" class="rowimg" alt="">
                        <h3>IT柜</h3>
                      </div>
                      <div class="imdcrow-char-right" v-if="(i == 1 && table.rdpNum != 5 && table.rdpNum != 6) || (table.rdpNum > 4 && i == parseInt(table.rdpNum/2)+1)">
                        <img src="@/assets/img/imdcrow-kt.png" class="rowimg" alt="">
                        <h3>列间空调</h3>
                      </div>
                    </template>
    ...
    methods: {
          ...
          if (response.status == 0) {
            if (response.data != null) {
              ...
              for (var key in that.formdata) {
                ...
                that.checkOptions(key, that.formdata[key]);
              }
          ...
          for (const i in that.navItem) {
            for (const j in that.navItem[i].sndImdccodeList) {
              ...
              for (const n in that.navItem[i].sndImdccodeList[j].sndImdccodeList) {
                ...
                if (that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].code == 'A01T2B1' || that.navItem[i].sndImdccodeList[
                    j].sndImdccodeList[n].code == 'A01T2B2') {
                ...
                if (that.formdata.A01T2 == '1') {
                  ...
                  if (that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].code == 'A02T2B1' || that.navItem[i].sndImdccodeList[
                      j].sndImdccodeList[n].code == 'A02T2B3') {
                    that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].fill = false
                  }
                }
                ...
                if (that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].code == 'A03T1B1' || that.navItem[i].sndImdccodeList[
                    j].sndImdccodeList[n].code == 'A03T1B2') {
                  that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].fill = false
                }
                //if #1="无空调MINI", 则机柜数量只可选1
                if (that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].code == 'A03T2B2' || that.navItem[i].sndImdccodeList[
                    j].sndImdccodeList[n].code == 'A03T2B3') {
                  that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].fill = true
                ...
                if (that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].code == 'A02T1B1' || that.navItem[i].sndImdccodeList[
                    j].sndImdccodeList[n].code == 'A02T1B2') {
                  that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].fill = false
                }
              }
            }
          ...
          for (const i in that.navItem) {
            for (const j in that.navItem[i].sndImdccodeList) {
              ...
              for (const n in that.navItem[i].sndImdccodeList[j].sndImdccodeList) {
                ...
                if (that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].code == 'A01T2B2' || that.navItem[i].sndImdccodeList[
                    j].sndImdccodeList[n].code == 'A01T2B3') {
                  that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].fill = false
                }
                ...
                if (that.formdata.A01T2 == '2') {
                  ...
                  if (that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].code == 'A02T2B2' || that.navItem[i].sndImdccodeList[
                      j].sndImdccodeList[n].code == 'A02T2B3') {
                    that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].fill = false
                  }
                }
                ...
                if (that.formdata.A01T2 == '3') {
                  if (that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].code == 'A02T2B1' || that.navItem[i].sndImdccodeList[
                      j].sndImdccodeList[n].code == 'A02T2B2') {
                    that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].fill = true
                ...
                if (that.formdata.A02T2 == '2') {
                  ...
                  if (that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].code == 'A03T2B1' || that.navItem[i].sndImdccodeList[
                      j].sndImdccodeList[n].code == 'A03T2B2') {
                    that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].fill = false
                  }
                }
                ...
                if (that.formdata.A02T2 == '3') {
                  if (that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].code == 'A04T1B1' || that.navItem[i].sndImdccodeList[
                      j].sndImdccodeList[n].code == 'A04T1B2') {
                    that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].fill = false
                  }
                }
        ...
        for (const i in that.navItem) {
          for (const j in that.navItem[i].sndImdccodeList) {
            for (const n in that.navItem[i].sndImdccodeList[j].sndImdccodeList) {
              if (!e) {
                if (that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].code == 'A02T2B1' || that.navItem[i].sndImdccodeList[
                    j].sndImdccodeList[n].code == 'A02T2B2' || that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].code ==
                  'A02T2B3') {
                  that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].fill = false
                  that.formdata.A02T2 = '';
                  that.upsValOption('');
        ...
        for (const i in that.navItem) {
          for (const j in that.navItem[i].sndImdccodeList) {
            for (const n in that.navItem[i].sndImdccodeList[j].sndImdccodeList) {
              if (!e) {
                if (that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].code == 'A03T2B1' || that.navItem[i].sndImdccodeList[
                    j].sndImdccodeList[n].code == 'A03T2B2' || that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].code ==
                  'A03T2B3') {
                  that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].fill = false
                  that.formdata.A03T2 = ''
                }
      ...
      reloadImdc() {
        ...
        for (const i in that.navItem) {
          for (const j in that.navItem[i].sndImdccodeList) {
            // 盲板/导轨/层板不可填
            if (that.navItem[i].sndImdccodeList[j].code == 'A06T1' || that.navItem[i].sndImdccodeList[j].code ==
              'A06T2' || that.navItem[i].sndImdccodeList[j].code == 'A06T3') {
              that.navItem[i].sndImdccodeList[j].fill = true
            } else {
              that.navItem[i].sndImdccodeList[j].fill = false
            }
            for (const n in that.navItem[i].sndImdccodeList[j].sndImdccodeList) {
              // 设置一体式空调不可选  IEC口不可选
              if (that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].code == 'A04T2B2' || that.navItem[i].sndImdccodeList[
                  j].sndImdccodeList[n].code == 'A05T1B2') {
                that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].fill = true
              } else {
                that.navItem[i].sndImdccodeList[j].sndImdccodeList[n].fill = false
              }
