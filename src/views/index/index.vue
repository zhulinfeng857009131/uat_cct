  ...
  <div id="">
    ...
    <el-container class="cont-conar cont-jjfa" style="margin-top: 0;">
      ...
      <el-table v-loading="loadingf" ref="multipleTable" :data="tableData1" @selection-change="handleSelectionChange" style="width: 100%;box-sizing: border-box;">
        ...
        <el-table-column prop="isxNumber" label="ISX号"/>
...
export default {
  ...
  methods: {
    ...
    checkhandle(params) {
      ...
      const sndStatus = params.sndStatus
      const that = this
      let msg = ''
     if (modifyval == '审核通过' || modifyval == '审核不通过') {
      ...
      if (modifyval == null) {
        that.openModify(solutionID, sndStatus)
      //} else if (modifyval == '审核通过' || modifyval == '审核不通过') {
        ...
        this.$confirm(msg, '提示', {
          ...
          .then(() => {
            that.updateSolutionStatus(solutionID,sndStatus)
          })
          .catch(action => {
          })
      } else {
    ...
    updateSolutionStatus(solutionID,sndStatus) {
      const that = this
        const loading = this.$loading({
          lock: true,
          text: 'Loading',
    ...
    openModify(solutionID,sndStatus) {
      console.log(sndStatus)
      sndStatus = sndStatus.toString()
      let solutionType = 0 //imdc or gpx
      if(sndStatus && sndStatus.length > 0){
        //solutionType = sndStatus.indexOf('Galaxy PX') != -1 ? 'gpx' : sndStatus.indexOf('Mini&One') != -1 ? 'imdc' : '-1'
        if(solutionType = sndStatus.indexOf('Galaxy PX') != -1){
              sessionStorage.setItem('solutionType', 'gpx')
          this.$router.push({
        ...
        }else if(sndStatus.indexOf('Mini&One') != -1){
          this.$router.push({
            path: '/imdc/imdc-list',
            query: {
              solutionId: solutionID,
