<template>
  <div id="" style="min-width: 1300px;margin: 0 auto;">
    <el-container class="cont-conar cont-addjjfa" style="padding: 0 10px;min-height: 500px;">
      ...
      <el-row :gutter="20" style="display: none; padding: 20px 0;background-color: #fff;margin :0 1px 20px;border: 1px solid #cfcfcf;">
        <el-col :span="12">
          <div class="abox">
            <h3>
              <img src="@/assets/img/add-ico-1.png">
              基本信息
            </h3>
            <ul>
              <li>项目名称<span>{{ titleData.projectName }}</span></li>
              <li>Opp ID<span>{{ solutionInfo.oppID }}</span></li>
              <li>ISX Numbber<span>{{ solutionInfo.isxNumber }}</span></li>
              <li>施耐德销售<span>{{ decodeURIComponent(solutionInfo.realname) }}</span></li>
              <li>方案备注<span>{{ solutionInfo.remarks }}</span></li>
            </ul>
          </div>
        </el-col>
        <el-col :span="12">
          <div class="abox">
            <h3>
              <img src="@/assets/img/add-ico-2.png">
              状态信息&总价
            </h3>
            <ul>
              <li>方案状态<span>
                  <template v-if="solutionStatus.falg == 0">正常</template>
                  <template v-if="solutionStatus.falg == 5">草稿</template>
                  <template v-if="solutionStatus.falg == 9">删除</template></span>
              </li>
              <li>方案审核状态<span class="r">{{ solutionStatus.checkStatus }}</span></li>
              <!-- <li>Design Portal 状态<span class="r">{{ solutionStatus.designPortalStatus }}</span></li> -->
              <li>特价审核状态<span class="r">{{ solutionStatus.charterStatus }}</span></li>
              <li>方案总价<span> ￥{{ prices }}(CNY)</span></li>
              <li><span></span></li>
            </ul>
          </div>
        </el-col>
        <!-- <el-col :span="8">
          <div class="abox">
            <h3>
              <img src="@/assets/img/add-ico-3.png">
              价格
            </h3>
            <ul>
              <li>方案总价<span> ￥{{prices}}(CNY)</span></li>
      ...
      <el-row class="cct-col-divl" :gutter="20" style="padding: 10px 0;background-color: #a5a9ac;color:#fff;margin :0 1px 20px;border: 1px solid #cfcfcf;">
        <el-col :span="8">
          ...
          <div>施耐德销：<span>{{ decodeURIComponent(solutionInfo.realname) }}</span></div>
        </el-col>
        <el-col :span="8">
          <div>Opp ID：<span>{{ solutionInfo.oppID }}</span></div>
          <div>
            方案状态：<span>
              <template v-if="solutionStatus.falg == 0">正常</template>
              <template v-if="solutionStatus.falg == 5">草稿</template>
        ...
        <el-col :span="8">
          <div>施耐德销售：<span>{{ decodeURIComponent(solutionInfo.realname) }}</span></div>
          <div>方案总价：<span> ￥{{ prices }}(CNY)</span></div>
        </el-col>
      </el-row>
      <el-container>
        <el-aside v-if="modifyFlag == 'true'" width="250px" style="margin-right: 10px;">
          <el-menu :default-openeds="['1','11']" style="background-color: #F5F5F5;">
            <el-submenu index="1" style="border: 1px solid #cdcdcd;background-color: #fff;padding-bottom: 20px;">
              ...
              <el-submenu v-for="(item,index) in tableData" class="submenu" :default-openeds="['11']" :index="'1'+index"
                :disabled="item.content !='UPS及配电'&&item.content !='IMDC'?true:false">
                <!-- <template slot="title" class="submenu" ><span :style="{color:(item.content !='UPS及配电'&&item.content !='IMDC'?'grey !important':'')}">{{ item.content }}</span></template>
                <el-menu-item :disabled="item.content !='UPS及配电'&&item.content !='IMDC'?true:false" v-for="data in item.statuscodeList" @click="addSndSolution1(data.code)" >
					<span :style="{color:(data.content !='Galaxy PX'&&data.content !='IMDC Mini&One'?'grey !important':'')}">{{ data.content }}</span>
				</el-menu-item> -->
                <template slot="title" class="submenu"><span @click="addSndSolution1('A01T1B2')">{{ item.content }}</span></template>
                <el-menu-item :disabled="item.content !='UPS及配电'&&item.content !='IMDC'?true:false" v-for="data in item.statuscodeList"
                  @click="addSndSolution1(data.code)">
                  <span :style="{color:(data.content !='Galaxy PX'&&data.content !='IMDC Mini&One'?'lightgrey !important':'')}">{{ data.content }}</span>
                </el-menu-item>
              </el-submenu>
        ...
        <el-main style="padding: 0;height: auto; padding: 0 15px 15px; border: 1px solid rgb(205, 205, 205); background-color: rgb(255, 255, 255); margin-bottom: 20px;">
          ...
          <el-main style="padding:20px 0">
            ...
            <div style="float: left;margin-left:5%;width: 90%;">
              <el-table v-if="showQsku" :data="tableData1.filter(f => f.productNum > 0)" show-summary :summary-method="getSummaries"
                style="width: 100%;">
                <el-table-column :label="'QSKU : '+qsku">
                  ...
                  <el-table-column prop="ddpPrice" label="单价" width="150" style="text-align: center;">
                    <template slot-scope="scope">
                      ￥{{ scope.row.ddpPrice | formatPrice1 }}(CNY)
                    </template>
                  </el-table-column>
                </el-table-column>
              </el-table>
              <el-table v-else :data="tableData1.filter(f => f.productNum > 0)" show-summary :summary-method="getSummaries"
                style="width: 100%;">
                <el-table-column prop="sku" label="产品型号" width="160" style="text-align: center;" />
                <el-table-column prop="description" label="描述" width="" style="text-align: center;" />
                <el-table-column prop="productNum" label="数量" width="80" />
