<template>
  <el-container class="" style="width: 80%;margin-left: auto;margin-right: auto;">
    <el-form class="demo-ruleForm adds" style="width:100%;margin:0 auto;">
      <el-checkbox-group v-model="fileIds" @change="changeValue()">
        <el-form-item v-for="item in files">
          <el-checkbox :label="item.id" border>{{ item.fileName }}</el-checkbox>
        </el-form-item>
      </el-checkbox-group>
      <el-form-item style="text-align: center;padding: 20px 0;border-top:1px solid #EBEEF5 ;">
        <el-button v-if="showBtn" type="primary" @click="download()">下载</el-button>

        <!-- <a :href="url" @click="download()">下载</a> -->
      </el-form-item>
    </el-form>
...
export default {
  ...
  methods: {
    ...
    changeValue() {
      ...
      that.url = process.env.VUE_APP_BASE_API+'/api/product/snd-profile/uploadFile?fileIds=' + that.fileIds + '&token=' + getToken()
    },
    download() {
      var that = this
